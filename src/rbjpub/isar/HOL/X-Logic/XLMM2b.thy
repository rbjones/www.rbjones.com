...
text{*
In theory XLMM2
\footnote{$ $Id: XLMM2b.thy,v 1.1 2007/07/17 15:55:51 rbj01 Exp $ $}
...
datatype judgement =
    Assert stamp authority "authority set" sentence |
...
primrec
    "jstamp (Assert st a as se) = st"
primrec
    "jauth (Assert st a as se) = a"
    "jauth (Endorse a as) = a"
primrec
    "jauths (Assert st a as se) = as"
    "jauths (Endorse a as) = as"
primrec
    "jsent (Assert st a as se) = se"
...
constdefs
   ...
   "basett j ji == case j of
      ...
      (Assert n a as s) \<Rightarrow> truesen (sinterp ji) s"

   itrec :: "nat \<Rightarrow> (inftest * truthtest) \<Rightarrow> inftest"
   "itrec n tsts auth ji == \<forall>a. (auth,a) \<in> authr (judgements ji)
          \<longrightarrow> (\<forall>j. j \<in> {jm . jm \<in> judgements ji \<and> jauth jm = a} & jstamp j < n
...
inductive "thms H"
  ...
  E:  "\<lbrakk> H \<turnstile> Assert n1 a (ll Un levels1) sent;
         H \<turnstile> Endorse l ll;
         n1 < n3;
         n2 < n3 \<rbrakk>
       \<Longrightarrow> Assert n3 ({l} Un levels2) sent : thms H"

  TI: "\<lbrakk> H \<turnstile> Assert a1 n1 levels1 (TrueDocs [d] [l]);
         H \<turnstile> Assert a2 n2 levels2 (TrueDocs dl ll);
         n1 < n3;
         n2 < n3 \<rbrakk>
       \<Longrightarrow> Assert a n3 (levels1 Un levels2) (TrueDocs (d#dl) (l#ll)):thms H"
