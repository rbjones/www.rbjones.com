...
\ignore{
  For convenience in referring to the philosophical position described here, I have adopted the term \emph{metaphysical positivism}.
I believe the term has been used before, but for present purposes this term is a neologism, not intended to refer to any previous usage.

A distinguishing feature of positivism since Hume is the rejection of ``metaphysics'', though it is clear in Hume that it is only \emph{metaphysics} which is to be rejected in favour of more acceptable forms.
It may therefore appear that the admixture of positivism with metaphysics is a radical departure from the tradition, so I begin this sketch with a few words to explain how modest an innovation is intended.

There is already in Rudolf Carnap an innovation which takes us almost as for as we need go, for Carnap's rejection of \emph{metaphysics} was not a denial of metaphysical claims but rather the rejection of such claims as ``meaningless'', later to be moderated to ``lacking cognitive content''.
Alongside this, Carnap allowed that many of the statements which might otherwise be regarded as metaphysical explicitly or as involving implicit metaphysical presumption can be rehabilitated and rendered meaningful by clarification of context.

\section{}

Metaphysical positivism is primarily epistemological.
It is concerned with certain kinds of knowledge which we call \emph{propositional} knowledge, the kind of knowledge which can be expressed in language as propositions.
It is particularly concerned with knowledge as shared resource which helps us to live and prosper.

Wittgenstein's later philosophy emphasised the diverse ways in which language is used, among which the expression of propositions is but one special case.
Nevertheless, it is a very important case, and

}%ignore

Positive philosophy may be considered as falling into two parts, using terminology similar to that of Aristotle.
The two parts are respectively \emph{theoretical} and \emph{practical}, and though the words come from Aristotle, the line between them as I use them, is probably better seen in the second of Hume's ``forks'' in which he tells us ``you cannot derive an ought from an us''.
``Metaphysical'' positivism is concerned only with what is,  how it works, how to change it, how to build and adapt.
``Practical'' positivism concerns all that involves the values which enable us to choose, and the world we build on the values.

Its time now to put the story together.
