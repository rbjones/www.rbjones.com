...
<xsl:variable name="flfn" select="concat($name,'.',$suffix)"/>
<xsl:variable name="ilfn" select="concat($name,'-i.',$suffix)"/>
<xsl:variable name="mlfn" select="concat($name,'-m.',$suffix)"/>
<xsl:variable name="ffn" select="concat($path,$flfn)"/>
<xsl:variable name="ifn" select="concat($path,$ilfn)"/>
...
<xsl:output name="frame" method="xhtml" version="1.0" encoding="US-ASCII"/>
<xsl:output name="index" method="xhtml" version="1.0" encoding="US-ASCII"/>
...
<xsl:include href="xhtmlinxld2.xsl"/>
...
<xsl:template match="xld:xldoc">
 ...
 <xsl:result-document href="{$ffn}" format="frame">
 <xsl:message><xsl:value-of select="concat('$class=',$class, ' @class=',@class)"/></xsl:message>
 <xsl:message><xsl:value-of select="concat('***[1]', $flfn)"/></xsl:message>
	<xsl:call-template name="xldframe"/>
 </xsl:result-document>

 <xsl:result-document href="{$ifn}" format="index">
